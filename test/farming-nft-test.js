...
describe("NFT Farming Contract", function () {
  ...
  it("should create simple farm with nft staking", async function () {
    try {
      const seed = await farmingContract.get_seed_info({
        seed_id: `${ftContractName}$1`,
      });
      const farm = await farmingContract.get_farm({
        farm_id: `${ftContractName}$1#0`,
      });
      console.log(seed);
      console.log(farm);
      if (seed) return;
      await farmingContract.create_simple_farm(
        {
          ...
          nft_balance: {
            "dev-1649259045266-73594760682734@1": "200000000000000000000",
            "dev-1649259045266-73594760682734@2": "200000000000000000000",
          },
          metadata: {
  ...
  it("should nft stake", async function () {
    try {
      const token_id = "1:1";
      const seed_id = `${ftContractName}$1`;
      ...
      if (
        user_nft_seeds[seed_id] &&
        user_nft_seeds[seed_id].includes(token_id)
      ) {
